...
静态变量和静态代码块的初始化在类被第一次加载的时候由JVM执行，无需程序员干预，且只会执行一次；实例变量和非静态代码块的初始化在使用关键字new创建对象的时候会被执行，且每创建一个对象便会执行一次；静态变量和实例变量都有默认值，而局部变量没有，因此局部变量必须先赋值在访问。

静态变量的生命周期处于类的加载到类的卸载这一时间范围；实例变量的生命周期处于创建对象到对象不可达被 GC 回收这一时间范围；局部变量的生命周期只在代码块或方法内有效。
...
- `int`
  ...
  > 默认情况下，int 数据类型是一个 32 位有符号二进制补码整数。
- `long`
  
  > long 数据类型是 64 位二进制补码整数。 signed long 的最小值为。
- `float`
  
  > 定义一个单精度浮点数变量，占 4 个字节，Java 中的字面量浮点数默认是 double 类型，因此想要表示 float 类型需要在浮点数后加上 F/f。
- `double`
  
  > 定义一个双精度浮点数变量，占 8 个字节。
- `char`
...
一个 float 类型的浮点数必须以 F 或 f 结尾，或者它会被认为是一个 double 类型的浮点数，double 类型的浮点数结尾可选择使用 D 或 d 标记。浮点数可以使用科学符号 E 或 e 表示。示例如下：
...
数组是 Java 语言内置的引用数据类型。

数组的访问从索引 0 开始，可以通过数组名.length 获取数组的长度。如果访问的索引超过了数组的索引范围，那么会抛出 索引越界异常。

可以在定义数组时直接指定初始化的元素，这样就不必写出数组大小，而是由编译器自动推算数组大小。

数组的特点：

- 数组所有元素初始化为默认值，整型都是`0`，浮点型是`0.0`，布尔型是`false`，引用类型都是 `null`
- 数组一旦创建后，大小就不可改变
...
- 文档注释（Java Doc）
  ...
  类、方法、字段推荐使用这种风格的注释，可以使用 javadoc 工具生成 html 格式的文档注释。
...
> Java定义了位运算符，只能应用于整数类型(int)，长整型(long)，短整型(short)，字符型(char)，和字节型(byte)类型。
...
break 和 continue 都是用再循环体中的，其中`break`语句总是跳出最近的一层循环，continue 提前结束本次循环。

而 return 则表示方法的调用结束了，并且返回方法调用的结果。
