...
封装的思想是将类向外暴露的信息尽可能的少，即只暴露需要提供给客户端的方法，将这些方法的实现细节使用 private 封装在类的内部，从而保护类的 API 功能正常。另一方面是将对象的状态数据封装在类的内部，只能通过合理地、正确的 API 进行对象状态的修改。封装体现了 OOP 编程中的高内聚。
...
多态是同一个行为具有多个不同表现形式或形态的能力。多态就是同一个接口（方法），使用不同的实例执行具有不同的结果。
