...
In this guide you are going to learn about the main pillars of [Spring core’s](https://www.marcobehler.com/guides/spring-framework) *transaction abstraction framework* (a confusing term, isn’t it?) - described with a lot of code examples:
...
As opposed to, say, the [official Spring documentation](https://docs.spring.io/spring-framework/docs/5.3.x/reference/html/data-access.html#transaction-declarative), this guide won’t confuse you by diving right into the topic *Spring-first*.

Instead you are going to learn Spring transaction management the *unconventional way*: From the ground up, step by step. This means, starting with plain old [JDBC transaction](https://en.wikipedia.org/wiki/Java_Database_Connectivity) management.
...
1. You need a connection to the database to start transactions. [DriverManager.getConnection(url, user, password)](https://docs.oracle.com/javase/7/docs/api/java/sql/DriverManager.html) would work as well, though in most enterprise-y applications you will have a data source configured and get connections from that.
...
(A quick note for smarty-pants: Connection pool libraries like [HikariCP](https://github.com/brettwooldridge/HikariCP) might toggle the autocommit mode automatically for you, depending on the configuration. But that is an advanced topic.)
...
2. Nested transactions in Spring are just JDBC / database savepoints. If you don’t know what a savepoint is, have a look at [this tutorial](https://docs.oracle.com/javase/tutorial/jdbc/basics/transactions.html), for example. Note that savepoint support is dependent on your JDBC driver/database.

## How Spring’s or Spring Boot’s Transaction Management works

As you now have a good JDBC transaction understanding, let’s have a look at how plain, [core Spring](https://docs.spring.io/spring/docs/current/spring-framework-reference/core.html) manages transactions. Everything here applies 1:1 to [Spring Boot](https://spring.io/projects/spring-boot) and Spring MVC, but [more about that](https://www.marcobehler.com/guides/spring-transaction-management-transactional-in-depth#transactional-spring-boot) a bit later..
...
Compared with the [plain JDBC example](https://www.marcobehler.com/guides/spring-transaction-management-transactional-in-depth#plain-jdbc-example):
...
We will not go into detail on XML configuration in this guide, but you can use this example as a starting point to dive deeper into it - if needed (taken straight from the [official Spring documentation](https://docs.spring.io/spring/docs/4.2.x/spring-framework-reference/html/transaction.html#transaction-declarative)):
...
You are specifying an [AOP advice](https://docs.spring.io/spring/docs/4.3.x/spring-framework-reference/htmlsingle/#aop-introduction) (Aspect Oriented Programming) with the above XML block, that you can then apply to your UserService bean like so:
...
- And then Spring is smart enough to transparently handle transactions for you: Any bean’s *public* method you annotate with the @Transactional annotation, will execute *inside a database transaction* (note: there are some [pitfalls](https://www.marcobehler.com/guides/spring-transaction-management-transactional-in-depth#transactional-pitfalls)).
...
Armed with the knowledge from the [JDBC transaction example](https://www.marcobehler.com/guides/spring-transaction-management-transactional-in-depth#plain-jdbc-example), the @Transactional UserService code above translates (simplified) directly to this:
...
It does that through a method called *proxy-through-subclassing* with the help of the [Cglib library](https://github.com/cglib/cglib). There are also other ways to construct proxies (like [Dynamic JDK proxies](https://docs.oracle.com/javase/8/docs/technotes/guides/reflection/proxy.html)), but let’s leave it at that for the moment.
...
Database isolation levels are, however, a complex topic, and you should take some time to fully grasp them. A good start is the official Postgres Documentation and their section on [isolation levels](https://www.postgresql.org/docs/9.5/transaction-iso.html).
...
There’s some tricks (like [self-injection](https://stackoverflow.com/questions/43280460/spring-self-injection-for-transactions/43282215)), which you can use to get around this limitation. But the main takeaway is: always keep the proxy transaction boundaries in mind.
...
**The only difference with Spring Boot is, however, that it automatically sets the `*@EnableTransactionManagement*` annotation and creates a `*PlatformTransactionManager*` for you - with its JDBC auto-configurations. Learn more about [auto-configurations here](https://www.marcobehler.com/courses/27-spring-core-masterclass).**
...
At some point, you will want your Spring application to integrate with another database library, such as [Hibernate](https://hibernate.org/) (a popular JPA-implementation) or [Jooq](https://www.jooq.org/) etc.
...
I**nstead of using a [DataSourcePlatformTransactionManager](https://docs.spring.io/spring-framework/docs/current/javadoc-api/org/springframework/jdbc/datasource/DataSourceTransactionManager.html) in your Spring configuration, you will be using a [HibernateTransactionManager](https://docs.spring.io/spring-framework/docs/current/javadoc-api/org/springframework/orm/hibernate5/HibernateTransactionManager.html) (if using plain Hibernate) or [JpaTransactionManager](https://docs.spring.io/spring-framework/docs/current/javadoc-api/org/springframework/orm/jpa/JpaTransactionManager.html) (if using Hibernate through JPA).**
...
For other integrations or a more in-depth understanding, it helps to have a quick look at all possible [PlatformTransactionManager](https://docs.spring.io/spring-framework/docs/current/javadoc-api/org/springframework/transaction/PlatformTransactionManager.html) implementations that Spring offers.
